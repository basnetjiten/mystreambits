    ...
    <div class="container">
        ...
        <div class="banner">
            ...
            @if ($user->service() == 'twitch')
                <a href="https://twitch.tv/{{ $user->name }}" class="link" target="_blank"><i class="fa fa-twitch"></i>
                    /{{ $user->name }}</a>
            @elseif ($user->service() == 'youtube')
                <a href="https://youtube.com/channel/{{ $user->service_id() }}" class="link" target="_blank"><i
                            class="fa fa-youtube-play"></i> /channel/{{ $user->name }}</a>
            @elseif ($user->service() == 'mixer')
                <a href="https://mixer.com/{{ $user->name }}" class="link" target="_blank"><i class="fa fa-xing"></i>
                    /id{{ $user->service_id() }}</a>
            @endif
        </div>
        {{-- Content --}}
        <div class="content">
            ...
            <div id="payForm">
                {{-- Back --}}
                <button class="back"><i aria-hidden="true"
                                        class="fa fa-long-arrow-left"></i> @lang('donations.donate.back')</button>
                {{-- Total --}}
...
@section('scripts')
            ...
            $('#postDonate').ajaxForm({
                ...
                success: function (data) {
                    ...
                    if (typeof data.id != 'undefined') {
                        ...
                        /* $('.payment-methods a').each(function (i,m) {
                             //$('#myInput').val(55);
                             let id = $('#'+m).attr('id');
    ...
    <script src="{{asset('js/khalti.js')}}">
    </script>
    <script src="{{asset('js/esewa.js')}}">
    </script>
    <script src="{{asset('js/imepay.js')}}">
    </script>
@endsection
